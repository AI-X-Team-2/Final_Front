...
const WordGame = () => {
  ...
  const gameLoop = useCallback((timestamp) => {
    ...
    currentWords.current = currentWords.current.filter((w) => {
      ...
      if (w.y >= canvas.height - 130) { // 빨간 선의 위치를 100에서 50으로 내린 것에 맞춰 조건 변경
        setLives((prev) => {
          const next = prev - 1;
  ...
  return (
    <div ref={containerRef} className="relative w-full h-[40rem] flex items-center justify-center mt-10">
